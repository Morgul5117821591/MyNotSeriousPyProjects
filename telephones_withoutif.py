...
# Очистка номеров с использованием функции clean_and_count
phone_book = list(map(clean_and_count, phone_book))
print('phone_book =', phone_book, type(phone_book))

# Фильтрация номеров по длине
...
# Преобразование номеров в стандартный формат
m1 = map(lambda x: (x[0], '+7495' + x[1]), shorts)
m2 = map(lambda x: (x[0], '+7' + x[1][1:]), start_with_8)

# Объединение и сортировка списка номеров
...
# Проверка совпадений и генерация ответа
